<template>
         ...
         <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden
           ...
           <div class="py-4 text-left px-6">
             ...
             <div class="flex justify-between items-center pb-4">
               <p class="text-2xl font-poppins font-bold ">Change Password</p>
               <!-- Modal Close Button -->
               <div class="modal-close cursor-pointer z-50">
                 <router-link :to="{ name: 'clientprofile' }" class="text-[#B67B46]"><i class="fas fa-times color-black"></i>
                 </router-link>
   ...
   <vee-form :validation-schema="schema" @submit="handlesubmit">
    ...
    <div class="mb-3">
           <span class="z-10 h-full leading-snug font-normal absolute text-center text-slate-300
           bg-transparent rounded text-base items-center justify-center w-8 pl-3 py-2">
           <i class="fas fa-lock"></i></span>
         <vee-field type="password" v-if="show" placeholder="Current Password" v-model="currentPassword" name="currentpassword"
         ...
         <vee-field type="text" v-else placeholder="Current Password" v-model="currentPassword" name="currentpassword"
          class="px-40 py-2 placeholder-slate-300 text-slate-600 relative  bg-white rounded text-sm border border-slate-300 outline-none
         focus:outline-none focus:border-one focus:ring-0  pl-10 " />
       ...
       <div class="mb-3">
           ...
           bg-transparent rounded text-base items-center justify-center w-8 pl-3 py-2">
           <i class="fas fa-lock"></i></span>
         <vee-field type="password" v-if="showPassword" placeholder="New Password" v-model="newPassword" name="password"
         class="px-40 py-2 placeholder-slate-300 text-slate-600 relative  bg-white rounded text-sm border border-slate-300 outline-none
         focus:outline-none focus:border-one focus:ring-0  pl-10 " />

         <vee-field type="text" v-else placeholder="New Password" v-model="newPassword" name="password" class="px-40 py-2 placeholder-slate-300 text-slate-600 relative bg-white bg-white rounded text-sm border border-slate-300 outline-none
         focus:outline-none focus:border-one focus:ring-0  pl-10 " />
         <span class=" h-full font-normal absolute text-center text-slate-300
               bg-transparent rounded
          text-base items-center justify-center  right-10 pt-1.5  ">
       ...
       <div class="mb-3">
         ...
         <span class="z-10 h-full leading-snug font-normal absolute text-center text-slate-300
           ...
           v-model="confirmPassword" class="px-40 py-2 placeholder-slate-300 text-slate-600 relative  bg-white rounded
          text-sm border border-slate-300 outline-none focus:outline-none focus:border-one focus:ring-0
           w-full pl-10" />
         <vee-field type="text" v-else placeholder="Confirm Password" name="confirm_password" v-model="confirmPassword" class="px-40 py-2 placeholder-slate-300 text-slate-600 relative  bg-white rounded
    ...
    text-xl font-poppins mt-2 w-full h-10 text-center
      rounded-xl   text-white focus:ring-0
         hover:text-[#75491d] ">
           Reset
         </button>
         <hr />
    ...
    //  import homePage from '../view/homePage.vue';
     ...
     export default {
      ...
      schema: {
        ...
        currentpassword: 'required|min:6|max:100' ,
    ...
    this.$store.dispatch("auth/resetpassword", reset)
